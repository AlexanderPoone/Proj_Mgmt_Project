<<<<<<< HEAD:material-kit-react-main/main.py
##############################################
#
...
# Get contributors' names and avatars
def getContributors(owner, reponame, tok):
	url = f'https://api.github.com/repos/{owner}/{reponame}/contributors'
	print(url)
	print(tok)
	req = Request(url)
...
class JSONEncoder(json.JSONEncoder):
    def default(self, o):
        if isinstance(o, ObjectId) or isinstance(o, datetime): return str(o)
...
## Front End Use
# Assign a collaborator to a team
@app.route('/assignTeam/<string:owner>/<string:reponame>/<string:collaborator>/<string:role>', methods = ['GET'])
def assignTeam(owner, reponame, collaborator, role):
	collection = db['roles']

	collection.update_one({'owner': owner, 'reponame': reponame, 'collaborator': collaborator}, {'$set': {'role': role}})

	return json.dumps({'owner': owner, 'reponame': reponame, 'collaborator': collaborator, 'role': role})
...
def confirm(owner, reponame, issue_number, assignee, startdate, numdays):
	...
	return json.dumps({
		 ...
		 'originalenddate': enddate.strftime('%Y-%m-%d'),
		 'assignee': assignee,
		 'status': 'normal'
		})
...
>>>>>>> 218605b596b97c8cdaaea7ef280824f118d58e63:flask/draft.py
