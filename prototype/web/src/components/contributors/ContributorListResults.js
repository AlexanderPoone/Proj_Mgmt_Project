...
const ContributorListResults = ({ contributors, ...props }) => {
  ...
  const handleSelectAll = (event) => {
    ...
    if (event.target.checked) {
      newSelectedCustomerIds = contributors.map((contributor) => contributor.id);
    } else {
            ...
            <TableBody>
              {contributors.map((contributor) => (
                <TableRow
                  hover
                  key={contributor.id}
                  selected={selectedCustomerIds.indexOf(contributor.id) !== -1}
                  onClick={event => { props.handleRowClick(event, contributor) }}
                >
                  {/* <TableCell padding="checkbox">
                    <Checkbox
...
ContributorListResults.propTypes = {
  customers: PropTypes.array.isRequired
};

export default ContributorListResults;
